...
- **.NET engine — `DeleteQsoAsync` parity.** The .NET `QrzLogbookClient` does not yet expose a delete helper matching the Rust client's semantics; currently local-only deletes are fully supported but remote `ACTION=DELETE` is not wired. This is being replaced with a soft-delete-with-restore design (see §7.8 — planned).
